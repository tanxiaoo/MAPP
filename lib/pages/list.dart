...
class _ListPageState extends State<ListPage> {
  ...
  Future<void> _loadPlans() async {
    ...
    if (userId != null) {
      List<Map<String, dynamic>> plans = await firestoreService.fetchSavedPlans(userId);
      setState(() {
        _plans = plans;
      });
    }
  ...
  Future<void> _updatePlanOrder(int oldIndex, int newIndex) async {
    String? userId = getCurrentUserId();
    ...
    setState(() {
      ...
      final Map<String, dynamic> plan = _plans.removeAt(oldIndex);
      _plans.insert(newIndex, plan);
    });

    // 更新 Firebase 中的顺序
    await firestoreService.updatePlanOrder(userId, _plans);
  }
  ...
  Widget _buildPlanCard(Map<String, dynamic> plan, int index) {
    ...
    if (plan["date"] != null && plan["date"] != "Not selected") {
      try {
        ...
        if (dates.length == 2) {
          ...
          formattedDateRange =
              "${DateFormat("ddMMyyyy").format(startDate)} - ${DateFormat("ddMMyyyy").format(endDate)}";
        }
      } catch (e) {
        // Handle error
      }
    ...
    return Card(
      key: Key('plan_$index'), // 为每个卡片设置唯一的 key
  ...
  Widget _buildTransportDetails(int index, Map<String, dynamic> routeDetails, Map<String, dynamic> selectedTickets) {
              ...
              child: isTablet
                  ...
                  : ReorderableListView(
                      ...
                      onReorder: (oldIndex, newIndex) {
                        _updatePlanOrder(oldIndex, newIndex);
                      },
                      children: List.generate(_plans.length, (index) {
                        final plan = _plans[index];
